...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .ai-auth { max-width: 600px; margin: 2rem auto; padding: 2rem; background: var(--bg1); border-radius: 8px; }
    .ai-auth h2 { color: var(--accent); margin-bottom: 1rem; }
    .auth-buttons { margin: 1.5rem 0; }
    .google-btn, .pro-btn, .logout-btn { width: 100%; padding: 1rem; border-radius: 8px; border: none; font-size: 1rem; cursor: pointer; margin-bottom: 1rem; }
    .google-btn { background: #4285f4; color: white; }
    .pro-btn { background: var(--accent); color: white; }
    .logout-btn { background: var(--bg3); color: var(--text0); }
    .or-divider { text-align: center; margin: 1rem 0; color: var(--text2); }
    .email-signup { display: flex; flex-direction: column; gap: 0.75rem; }
    .email-signup input, .email-signup select { padding: 0.75rem; border-radius: 8px; border: 1px solid var(--bg3); background: var(--bg0); color: var(--text0); font-size: 1rem; }
    .email-signup button { padding: 0.75rem; border-radius: 8px; border: none; background: var(--accent); color: white; font-size: 1rem; cursor: pointer; }
    #auth-result { margin-top: 1rem; }
    #auth-result .success { color: var(--accent); }
...
document.addEventListener('keypress', function(e) {
  if (e.key === 'Enter') {
    ...
    if (document.getElementById('email-input')) signup();
  }
});
...
async function checkAuth() {
  try {
    ...
    if (data.loggedIn) {
      ...
      document.getElementById('user-plan').textContent = data.plan === 'pro' ? 'Pro ($5/mo)' : 'Starter ($2/mo)';
      if (data.plan !== 'pro') {
        document.getElementById('plan-upgrade').style.display = 'block';
      }
    }
  } catch (e) {}
}
